...
import { AlertCircle, ZoomIn, ZoomOut, ChevronLeft, ChevronRight, RotateCcw } from "lucide-react"
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert"
import { useMobile } from "@/hooks/use-mobile"

export function ResponsiveECGVisualization() {
  ...
  }, [ecgData, zoom, position, selectedPeakIndex])
  ...
  const drawRPeaks = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // Draw R-peaks
    for (let i = 0; i < ecgData.rPeaks.length; i++) {
      ...
      if (peakIdx >= startIdx && peakIdx < endIdx) {
        ...
        if (selectedPeakIndex === i) {
          ctx.fillStyle = "rgba(249, 115, 22, 0.7)" // Orange for selected peak
          ctx.beginPath()
          ctx.arc(x, y, 7, 0, 2 * Math.PI)
          ctx.fill()
        }

        ctx.fillStyle = "rgba(220, 38, 38, 0.7)" // Red for normal peaks
        ctx.beginPath()
  ...
  const navigatePeaks = (direction: "prev" | "next") => {
    if (!ecgData?.rPeaks || ecgData.rPeaks.length === 0) return

    let newIndex: number

    if (selectedPeakIndex === null) {
      // If no peak is selected, select the first or last peak
      newIndex = direction === "next" ? 0 : ecgData.rPeaks.length - 1
    } else {
      // Move to the next or previous peak
      newIndex =
        direction === "next"
          ? Math.min(selectedPeakIndex + 1, ecgData.rPeaks.length - 1)
          : Math.max(selectedPeakIndex - 1, 0)
    }

    setSelectedPeakIndex(newIndex)

    // Center the view on the selected peak
    if (ecgData.rawData) {
      const peakPosition = ecgData.rPeaks[newIndex] / ecgData.rawData.length
      setPosition(Math.max(0, Math.min(peakPosition - 0.1, 0.9))) // Center with some margin
    }
  }
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="relative border rounded-lg overflow-hidden">
            ...
            {ecgData.rPeaks && ecgData.rPeaks.length > 0 && (
              <div className="absolute bottom-2 right-2 flex space-x-2">
                <Button variant="outline" size="icon" onClick={() => navigatePeaks("prev")}>
                  <ChevronLeft className="h-4 w-4" />
                </Button>
                <Button variant="outline" size="icon" onClick={() => navigatePeaks("next")}>
          ...
          {selectedPeakIndex !== null && ecgData.rPeaks && (
            <div className="text-sm">
              Selected R-peak: {selectedPeakIndex + 1} of {ecgData.rPeaks.length}
              (position: {ecgData.rPeaks[selectedPeakIndex]})
            </div>
          )}
        </CardContent>
      </Card>
